...
class CobrancasCompA6Pipeline(CobrancasPipeline):
    """A6-specific cobrancas competência pipeline."""
    ...
    def executar(self, max_paginas: int, dt_inicio: str, dt_fim: str):
        ...
        return self.carregar_para_postgres(
            df=df,
            tabela="splgc-cobrancas_competencia-A6-2025-2026",
            modo='replace',
            chave_unica='comp.id_composicao_comp',
            script_rodado=f"Pipeline de cobranças por competência A6, {dt_inicio} a {dt_fim}"
        )


def main_cobrancas_comp_a6():
    access_token = Config.get_token("a6")
    db_connection = Config.DB_CONNECTION_STRING

    try:
        pipeline = CobrancasCompA6Pipeline(db_connection, access_token)
        ...
        print(f"Pipeline de cobranças competência A6: {status}")
