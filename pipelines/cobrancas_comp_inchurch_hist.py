...
class CobrancasCompInchurchHistPipeline(CobrancasPipeline):
    """Inchurch-specific cobrancas competência historical pipeline."""
    ...
    def executar(self, max_paginas: int, dt_inicio: str, dt_fim: str, modo: str = 'append', script_rodado: str = ''):
        ...
        """
        logger.info("🚀 Iniciando pipeline de cobranças por competência Inchurch histórico")
...
def main_cobrancas_comp_inchurch_hist(year: int, month: int):
    access_token = Config.get_token("inchurch")
    db_connection = Config.DB_CONNECTION_STRING

    try:
        pipeline = CobrancasCompInchurchHistPipeline(db_connection, access_token)
        ...
        if month != 0:
            ...
            sucesso = pipeline.executar(
                ...
                dt_fim=dt_fim,
                modo='append',
                script_rodado=f"Pipeline de cobranças por competência Inchurch histórico, {dt_inicio} a {dt_fim}"
            )
            status = "✅ Sucesso" if sucesso else "❌ Falha"
            print(f"Pipeline de cobranças {dt_inicio} a {dt_fim}: {status}")
            return

        # month == 0: loop bimestral, replace na primeira carga (2021)
        for mes in range(1, 12, 2):
            ...
            modo = 'replace' if dt_inicio == "01/01/2021" else 'append'
